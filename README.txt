...
A **Round-Robin Arbiter** is used in hardware systems to fairly distribute access to a shared resource among multiple requesters. Unlike fixed-priority arbitration, round-robin arbitration prevents starvation by rotating priority after each grant.
...
Represents a randomized test stimulus including:
...
Implements the **golden arbitration algorithm** used to calculate the expected grant.
...
Coverage ensures the arbiter is exercised across different scenarios:
...
5. Compiles the top-level testbench
6. Starts simulation
7. Adds useful waveforms
...
- Conversion to a full **UVM verification environment**
- Adding a `.gitignore` to exclude simulation artifacts
